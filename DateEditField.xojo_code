...
Inherits TextField
		...
		Sub TextChange()
		  ...
		  if len(me.text)<=MaxLength then
		    ...
		    if not(DeleteKey) and not(BackSpace) then
		      ...
		      if (me.SelStart>0) and (me.SelLength>0) and ((asc(me.SelText) < 48) or (asc(me.SelText) > 57)) then 'not a number
		        DataEntryError=true
		      end if
	...
	#tag Method, Flags = &h21
		Private Function ValidDay() As Boolean
	...
	#tag Method, Flags = &h21
		Private Function ValidMonth() As Boolean
	...
	#tag Constant, Name = MaxLength, Type = Double, Dynamic = False, Default = \"10", Scope = Public
	#tag EndConstant
